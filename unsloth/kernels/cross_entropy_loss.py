...
# ==============================================================================
# Implementation of Dynamic Fine-Tuning (DFT)
# Reference:
# Wu, Y., Zhou, Y., Ziheng, Z., et al. (2026). 
...
class Fast_CrossEntropyLoss(torch.autograd.Function):
    ...
    def forward(
        ...
        # --- DFT MODIFICATION (Forward) ---
        # Recover prob: p = exp(-CE_loss). detach() acts as stop-gradient (sg).
        target_probs = torch.exp(-losses).detach()
        losses = target_probs * losses

        ctx.save_for_backward(logits, logsumexp, labels, target_probs)
    ...
    def backward(ctx, dlosses):
        logits, logsumexp, labels, target_probs = ctx.saved_tensors
        n_rows: int
        vocab_size: int
        n_rows, vocab_size = logits.shape

        # --- DFT MODIFICATION (Backward) ---
        # Scale the gradient by target_probs to cancel standard CE's (1/p) weighting.
        dlosses = dlosses * target_probs
