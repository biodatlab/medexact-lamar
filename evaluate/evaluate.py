...
2. Token F1: Token-level classification F1 with macro-averaging #only for non-overlapping tokens

Usage:
    python evaluate_new.py --gold_dir physionet.org/files/meddec/1.0.0/data --raw_text_dir physionet.org/files/meddec/1.0.0/raw_text --stats_csv physionet.org/files/meddec/1.0.0/stats.csv --split_file physionet.org/files/meddec/1.0.0/splits/val.txt --predictions postprocess/predictions/mbr_merged_3_rl.json --output postprocess/eval/mbr_merged_3_rl.json
"""
...
def assign_token_labels(tokens, annotations, is_gold=True):
    ...
    Tokens covered by exactly one valid annotation get that label.
    Tokens covered by two or more valid annotations are marked -1 (ambiguous)
    and excluded from the metric downstream.
    """
    counts = [0] * len(tokens)
    labels = [0] * len(tokens)
    ...
    for ann in annotations:
        ...
        label = parse_category(ann.get('category', 0)) if is_gold else int(ann.get('category', 0))
        if label not in VALID_CATEGORY_IDS:
            continue
        ...
        for i, token in enumerate(tokens):
            if token['start'] < end and token['end'] > start:
                ...
                if counts[i] == 1:
                    ...
                    labels[i] = -1  # ambiguous; stays -1 for any further spans too
...
def compute_token_f1_per_sample(gold_labels, pred_labels):
    ...
    Positions where the gold label is -1 (ambiguous overlap) are excluded.
    """
    # Exclude positions ambiguous in gold
    pairs = [(g, p) for g, p in zip(gold_labels, pred_labels) if g != -1]
    if not pairs:
        return 1.0

    gold_f, pred_f = zip(*pairs)

    all_labels = set(gold_f) | set(pred_f)
    all_labels.discard(0)
    all_labels.discard(-1)

    if not all_labels:
        return 1.0 if list(gold_f) == list(pred_f) else 0.0

    f1_scores = []
    for label in all_labels:
        tp = sum(1 for g, p in zip(gold_f, pred_f) if g == label and p == label)
        fp = sum(1 for g, p in zip(gold_f, pred_f) if g != label and p == label)
        fn = sum(1 for g, p in zip(gold_f, pred_f) if g == label and p != label)
        precision = tp / (tp + fp) if (tp + fp) > 0 else 0.0
        recall    = tp / (tp + fn) if (tp + fn) > 0 else 0.0
        f1 = 2 * precision * recall / (precision + recall) if (precision + recall) > 0 else 0.0
        f1_scores.append(f1)
